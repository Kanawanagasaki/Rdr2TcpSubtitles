...
function timer_callback()
    ...
    if tcpClient == nil then
        ...
        if 600 < reconnectCounter then
            tcp_connect("127.0.0.1", 32123)
            reconnectCounter = 0
        end
    else
        local chunk, err = tcpClient:receive(4)

        if chunk then
            local bytes = { string.byte(chunk, 1, -1) }
            local b1 = bytes[1]
            local b2 = (bytes[2] * 2^8) % 2^32
            local b3 = (bytes[3] * 2^16) % 2^32
            local b4 = (bytes[4] * 2^24) % 2^32

            local strLen = bit.bor(b1, b2, b3, b4);

            if strLen < 0 or 256 < strLen then
                tcp_close()
                return
            end
            ...
            if chunk then
                ...
                local _, wordsAmount = text:gsub(" ", "")
                local speechDuration = (wordsAmount + 1) / 200 * 60, 0.4, 2.5
                if speechDuration < 0.4 then speechDuration = 0.4 end
                if speechDuration > 2.5 then speechDuration = 2.5 end
